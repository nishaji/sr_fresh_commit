...
public class AddAwardActivity extends BaseActivity implements DatePickerDialog.OnDateSetListener {
    ...
    private void setuptoolbar() {
        setSupportActionBar(add_tool);
        add_tool.setTitle(" Add Award");
        add_tool.setTitleTextColor(0xffffffff);
    ...
    void onFabClick() {
        if(award_title.getText().toString().length() == 0||award_org.getText().toString().length() == 0||award_desc.getText().toString().length() == 0||award_du_text.getText().toString().length() == 0){
            ...
            databaseHelper.setAwards(Award.builder()
                    ...
                    .setDate(date.toString()).setCreateflag("1").setUpdateflag("1").setPostflag("0").setPutflag("1").build());
                     startActivity(new Intent(AddAwardActivity.this,AwardActivity.class));
        }
    }
